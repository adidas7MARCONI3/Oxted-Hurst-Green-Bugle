...
def test_crime_collector_deduplicates_incidents():
    from collectors.crime import CrimeCollector
    # Same crime returned twice (by id) plus one returned twice by persistent_id.
    mock_crimes = [
        {"id": 1, "category": "theft",
         "location": {"street": {"name": "High Street"}},
        ...
        {"id": 1, "category": "theft",
         "location": {"street": {"name": "High Street"}},
         "outcome_status": None, "month": "2026-05"},
        {"id": 2, "persistent_id": "abc", "category": "burglary",
         "location": {"street": {"name": "Station Road"}},
         "outcome_status": None, "month": "2026-05"},
        {"id": 3, "persistent_id": "abc", "category": "burglary",
         "location": {"street": {"name": "Station Road"}},
         "outcome_status": None, "month": "2026-05"},
    ]
    with patch("collectors.crime.httpx.get") as mock_get:
    ...
    assert len(result.items) == 2  # 4 in, 2 unique out
