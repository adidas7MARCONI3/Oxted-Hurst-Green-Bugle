...
def _find(el, name: str):
    """First direct child whose local tag name matches ``name`` (namespace-agnostic)."""
    if el is None:
        return None
    for child in el:
...
def _find_deep(el, name: str):
    """First descendant whose local tag name matches ``name`` (namespace-agnostic)."""
    if el is None:
        return None
...
def _text(el, tag: str) -> str:
    """Text of the first direct child matching ``tag`` by local name."""
    found = _find(el, tag)
...
class TrainsCollector(BaseCollector):
    ...
    def _fetch_station(self, crs: str, station_name: str) -> list[Item]:
        ...
        # Darwin returns StationBoard children across several versioned
        # ".../ldb/types" namespaces, so we match by local tag name rather than
        # binding to a single namespace (which silently parsed nothing before).
